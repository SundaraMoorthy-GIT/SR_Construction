...
export class SmsSettingComponent implements OnInit {
  ...
  get_SMS_Details()
  {
     this.appservice.getc("Api/Common/Get_SMS").subscribe((res:any )=>{
  ...
  addData(f)
    {
      ...
      this.http.post(this.appservice.Server_URL+'api/Common/insert_SMS_setting', f.form.value)
      .subscribe(
        (val:any) => {
