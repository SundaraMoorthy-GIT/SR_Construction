...
export class AddExpenseComponent implements OnInit {
  ...
  constructor(public datePipe :DatePipe,private _location: Location, public appservice: AppService, private toastr: ToastrService, private http: HttpClient, private router: Router, private route: ActivatedRoute) {
    ...
    if (this.isadd == "0") {
     ...
     try{
      ...
      if(this.add.oc_category=="Expenses")
        {
          this.Ledger=this.appservice.Reference_Rows.filter(e => e.Ref_ID == "Exp_Category");
        }
        else if(this.add.oc_category=="Employee")
        {
          this.Ledger=this.appservice.get_Ledger_group("4");
    
        }
        else if(this.add.oc_category=="Vehicle")
          {
            this.Ledger=this.appservice.Transport_Rows;
      
          }
          // else if(this.add.oc_category=="Project")
          //   {
  ...
  get_Receipt_no() {
    this.appservice.get("Api/Transaction/get_OtherCollection_No?Type=Expense").subscribe(( res: any) => {
      this.add.oc_no=res;
  ...
  get_Ledger_Category() {
   this.Exp_Category_ = this.appservice.Reference_Rows.filter(e => e.Ref_ID == "Exp_Group");
  }
