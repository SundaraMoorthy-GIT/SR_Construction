...
@Component({
  selector: 'app-fuel-maintanace-entry',
  templateUrl: './fuel-maintanace-entry.component.html',
  styleUrls: ['./fuel-maintanace-entry.component.scss']
})
export class FuelMaintanaceEntryComponent implements OnInit {
  ...
  constructor(public datePipe :DatePipe,private _location: Location, public appservice: AppService, private toastr: ToastrService, private http: HttpClient, private router: Router, private route: ActivatedRoute) {
    ...
    this.add.oc_type = "Vehicle Maintanace";
  ...
  onChange5(data)
  {
    this.add.oc_projectname=this.appservice.SProject_Rows.filter(e => e.value == data)[0].label; 
  }
  public refRow:any ={};
  onChange(data)
  {
    this.add.oc_category=this.Exp_Category_.filter(e => e.value == data)[0].label; 
  }

  onChange1(data)
  {
    this.add.oc_transport=this.appservice.Transport_Rows.filter(e => e.value == data)[0].label; 
  }
  onChange2(data)
  {
  ...
  onChange3(data)
  {
    this.add.oc_ledger_name=this.appservice.Ledger_Rows1.filter(e => e.value == data)[0].label; 
  }

  get_Receipt_no() {
    this.appservice.get("Api/Transaction/get_OtherCollection_No?Type=Vehicle Maintanace").subscribe(( res: any) => {
      this.add.oc_no=res;
  ...
  get_Ledger_Category() {
    try{
   this.Exp_Category_ = this.appservice.Reference_Rows.filter(e => e.Ref_ID == "Vehicle_Maintanace");
    }
    catch{}
  }
