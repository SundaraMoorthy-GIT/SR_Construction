...
@Component({
  selector: 'app-tipper-details',
  templateUrl: './tipper-details.component.html',
  styleUrls: ['./tipper-details.component.scss']
})
export class TipperDetailsComponent implements OnInit {
...
public btndisable:boolean=false;
  constructor(public confirmationService: ConfirmationService,public appservice:AppService, private router: Router, private route: ActivatedRoute) { 
    ...
    this.appservice.tpt_type="Tipper";
    this.appservice.get_Vehicle_Rpt("Tipper");
    this.appservice.get_Transport_Entry_Details();
    this.appservice.get_Party_details();
    this.appservice.get_From_Area();
  ...
  addReset() {
    ...
    return this.router.navigate(['/transport/add-tipper']);
  }
...
Delete(item) {
  ...
  export_excel(data)
  {
    this.appservice.Excel_Data.Header=this.appservice.Tipper_Details_GF ;
